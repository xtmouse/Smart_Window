...
static char *MakeProfileReport(WeChatProfile *payload)
{
    ...
    cJSON_AddNumberToObject(reported, payload->reportAction.subDeviceActionhw,
                            payload->reportAction.hwActionData);
    cJSON_AddNumberToObject(reported, payload->reportAction.subDeviceActionRain,
                            payload->reportAction.RainActionData);
    cJSON_AddNumberToObject(reported, payload->reportAction.subDeviceActionMQ2,
                            payload->reportAction.MQ2ActionData);
    ret = cJSON_PrintUnformatted(root);
    cJSON_Delete(state);
